...
class display:
    ...
    def __init__(self, setup, clock):
        ...
        # initial set of config elements
        ...
        # object specific sizes, directories and show dictionary objects
        # these could potentially be different for each initialized display
        # object in case w have multiple of these
        self.increment = 12
        ...
        if self.use_display == True:
            local_i2c = self.setup.getConfigParameter('oled_i2c')
            local_sda_pin = self.setup.getConfigParameter('oled_sda_pin')
            local_slc_pin = self.setup.getConfigParameter('oled_scl_pin')
            
            logger.trace('setting up I2C connection with i2c_bus ' + str(local_i2c) +' sda pin ' + str(local_sda_pin) + ' scl pin ' + str(local_slc_pin))
